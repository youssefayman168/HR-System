...
const EditEmployee = () => {
  ...
  const birthPah = data?.birth_certificate?.replace(
    "/media/birth-certificate/",
    ""
  );
  const employeePaper = data?.employee_paper?.replace(
    "/media/employees-paper/",
    ""
  );
  const graduationCertificate = data?.graduation_certificate?.replace(
    "/media/graduation-certificate/",
    ""
  );
  const idPhoto = data?.id_photo?.replace("/media/id-photo/", "");
  const militaryCertificate = data?.military_certificate?.replace(
    "/media/military-certificate/",
    ""
  );
  const passportCopy = data?.passport_copy?.replace(
    "/media/passport-copy-file/",
    ""
  );
  const scannedPaper = data?.scanned_paper?.replace(
    "/media/scanned_paper/",
    ""
  );
  const socialInsurance = data?.social_insurance?.replace(
  ...
  const editEmployee = useMutation({
    mutationFn: () => {
      return updateSingleEmployee(id, {
        ...
        graduation_year_from: employeeData.graduation_year_from
          ? employeeData.graduation_year_from
          : data?.graduation_year_from,
        graduation_year_to: employeeData.graduation_year_to
          ? employeeData.graduation_year_to
        ...
        birth_certificate: employeeData.birth_certificate
          ? employeeData.birth_certificate
          : data?.birth_certificate,
        employee_paper: employeeData.employee_paper
          ? employeeData.employee_paper
          : data?.employee_paper,
        graduation_certificate: employeeData.graduation_certificate
          ? employeeData.graduation_certificate
          : data?.graduation_certificate,
        id_photo: employeeData.id_photo
          ? employeeData.id_photo
          : data?.id_photo,
        military_certificate: employeeData.military_certificate
          ? employeeData.military_certificate
          : data?.military_certificate,
        passport_copy: employeeData.passport_copy
          ? employeeData.passport_copy
          : data?.passport_copy,
        scanned_paper: employeeData.scanned_paper
          ? employeeData.scanned_paper
          : data?.scanned_paper,
        social_insurance: employeeData.social_insurance
          ? employeeData.social_insurance
          ...
          <div className='mt-6 rounded-2xl bg-white px-8 h-[120px] '>
            <div className='flex items-center gap-4 h-full'>
              <div className='bg-[#F5F6F7] rounded-2xl text-xs flex items-center p-2 gap-2'>
                ...
                <span className='text-blackGrayColor font-bold'>
                  {data?.id}
                </span>
              </div>
            </div>
              ...
              <div className='w-[25%] border border-[#EFF1F4] rounded-2xl p-6 h-fit'>
                <div className='flex items-center justify-center'>
                  <div className='flex items-center gap-3 border-b border-b-[#F5F6F7] pb-6 relative'>
                    <img
                      className='rounded-full size-[80px] mx-auto'
                      src={`https://sec-system-apis.up.railway.app${data?.image}`}
                      alt='Picture'
                    />
                    <label
                      htmlFor='image'
                      className='size-[24px] absolute bottom-1 cursor-pointer right-[8px]'
                    >
                ...
                <div className='mt-8'>
                  <p className='text-primary text-xl font-bold mb-8'>
                    Personal Details
                  </p>
                  <EmployeeInfoInput
                    type='text'
              ...
              <div className='w-[25%] border border-[#EFF1F4] rounded-2xl h-fit p-6'>
                ...
                <div className='flex items-center gap-2 w-full'>
                  <EmployeeInfoInput
                    ...
                    type='date'
                    title='from'
                    value={data?.graduation_year_from}
                  />
                  <EmployeeInfoInput
                    inputStyle={"!w-[120px]"}
                    type='date'
                    title='to'
                    value={data?.graduation_year_to}
                  />
                </div>
              ...
              <div className='flex-1 border border-[#EFF1F4] rounded-2xl h-fit p-6'>
                ...
                <div className='flex flex-col ml-12 gap-9'>
                  ...
                  <EmployeeAttachment
                    ...
                    href={data?.birth_certificate}
                    type='Birth Certification'
                  />
                  <EmployeeAttachment
                    document={employeePaper}
                    documentType='Employee Paper'
                    href={data?.employee_paper}
                    type='Employee Paper'
                  />
                  <EmployeeAttachment
                    document={graduationCertificate}
                    documentType='Graduation Certificate'
                    href={data?.graduation_certificate}
                  ...
                  />
                  <EmployeeAttachment
                    document={militaryCertificate}
                    documentType='Military Certificate'
                    href={data?.military_certificate}
                    type='Military Certificate'
                  />
                  <EmployeeAttachment
                    document={passportCopy}
                    documentType='Passport Copy'
                    href={data?.passport_copy}
                    type='Passport Copy'
                  />
                  <EmployeeAttachment
                    document={scannedPaper}
                    documentType='Scanned Paper'
                    href={data?.scanned_paper}
                    type='Scanned Paper'
                  />
                  <EmployeeAttachment
                    document={socialInsurance}
                    documentType='Social Insurance'
                    href={data?.social_insurance}
