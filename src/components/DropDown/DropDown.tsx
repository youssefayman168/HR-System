...
type Drop = {
  ...
  text5?: string,
  style: CSSProperties,
  styleLast?: CSSProperties
}

const DropDown = ({ text1, text2, text3, text4, text5, style , styleLast }: Drop) => {

  return (
    <>
      <ul id='ullis' className='DropDownTable duration-300 w-[225px] text-start bg-white shadow-xl rounded-[10px] border-[1px] border-[#8F93AC] absolute left-0 top-[40px] z-[900] ' style={style}>
        ...
        <li style={styleLast} ><Link to={""} className='hover:bg-[#e4effe] duration-300 block mb-2 mx-2 py-2 ps-3 text-[black]'>{text5}</Link></li>
      </ul>
    </>
  )
}
